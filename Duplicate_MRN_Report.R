...
script_dir <- paste0("C:/Users/", username, "/Documents/GitRepositories/DuplicateMRN-GECB")
...
#do initial cleaning/transformation
...
setwd(paste0("C:/Users/", username, "/Box Sync/github_local/DuplicateMRN-GECB/input/BIDS Feed/dat_20180704"))
...
iq_regactivity_ca <- read_excel("InQuicker IDX Patients 20180704.xlsx", skip=3, col_names = ca_colnames, col_types = ca_coltypes)
...
shts <- excel_sheets("Duplicate IDX Patients - Standard_201807.xlsx")

#read through data on each sheet and put into a list to be bound together afterwards.
datalist.d1 = list()
for(i in 1:length(shts)) {
  dat <- read_excel("Duplicate IDX Patients - Standard_201807.xlsx", skip = 3, col_names = c("lastname", "firstname", "birthdate", "sex", "mrn", "ssn", "zip", "phone",
...
setwd(paste0("C:/Users/", username, "/Box Sync/github_local/DuplicateMRN-GECB/input/auxiliary_files"))
...
setwd(paste0("C:/Users/", username, "/Box Sync/github_local/DuplicateMRN-GECB/output/Report"))
#protect write-out
...
#add AZ
setwd(paste0("C:/Users/", username, "/Box Sync/github_local/DuplicateMRN-GECB/input/AZ Feed"))
iq_regactivity_az <- read_excel("InQuicker_MRN_Report_June.xls", skip = 1, col_names = c("mrn", "birthdate", "gender", "lastname", "firstname", "group",
...
iq_master_grped <- iq_master_grped %>% filter(yyyy_mm_dd > "2015-02-01" & yyyy_mm_dd <= "2018-07-31")
...
#####adhoc/troubleshoot#####
#evaluate my records against some samples that Linh provided on 4/5
#test_linh <- iq_regactivity_ca %>% filter(registrationdate >= "2018-03-07" & updated_or_created_target=="InQuicker" & baseline_duplication == "duplicate")
#find_linh <- iq_regactivity_ca %>% filter(lastname %in% c("CLARK", "ARAIZA", "GUAN") & firstname %in% c("BIANCA", "RICHARD", "SUNNY"))

#find_linh <- iq_regactivity_ca %>% filter((lastname=="CLARK" & firstname=="BIANCA") | (lastname=="ARAIZA" & firstname=="RICHARD") | (lastname=="GUAN" & firstname=="SUNNY"))

#setwd(paste0("C:/Users/", username, "/Box Sync/github_local/DuplicateMRN-GECB/output/auxiliary_files"))
#write.csv(test_linh, "dupes_from_20180307_20180411.csv", row.names=F)
#write.csv(find_linh, "review_linh_hits.csv", row.names=F)